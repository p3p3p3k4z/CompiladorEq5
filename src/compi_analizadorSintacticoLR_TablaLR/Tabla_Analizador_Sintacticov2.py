...
def InterfazTablaAS():
    ...
    VentanaPrincipal.title("Tabla de analisis sintactico LR")
    try:
        VentanaPrincipal.state("zoomed")
    except:
        VentanaPrincipal.attributes('-zoomed', True) 
    VentanaPrincipal.config(background="#15B392")
    font2=("Times New Roman",20)
    archivoLabel=Label(VentanaPrincipal,text="Seleccionar Archivo:",font=font1,width=20,background="#D2FF72",foreground="white")
    ...
    archivoButton=Button(VentanaPrincipal,text="Abrir archivo",width=20,command=lambda:abrirArchivo(VentanaPrincipal,frameGramatica,frameSiguientes),bg="#6256CA",font=font1)
    archivoButton.place(x=300,y=20)
    ImprimirResultad0s=Button(VentanaPrincipal,text="Imprimir tabla ",width=20,bg="#6256CA",font=font1,command=lambda:ImprimirTablaAS(frameTablaAS))
    ImprimirResultad0s.place(x=300,y=60)
    limpiarButton=Button(VentanaPrincipal,text="Limpiar",width=20,bg="#6256CA",font=font1,command=lambda:limpiar(frameTablaAS,frameGramatica,frameSiguientes))
...
direccionArchivo="../../pruebas_sintactico/gramaticas_tablaLR"
