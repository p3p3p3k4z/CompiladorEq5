...
def Prueba():
    ...
    archivoL = ctk.CTkLabel(lexWindow, text="Ingresa una prueba", width=200, font=font1)
    ...
    eregularL = ctk.CTkLabel(lexWindow, text="Expresión regular:", font=font1, width=200)
    eregularL.place(x=20, y=100)

    erEntry = ctk.CTkEntry(lexWindow, width=300, font=font1)
    erEntry.place(x=210, y=101)

    alphaLabel = ctk.CTkLabel(lexWindow, text="Alfabeto", font=font1, width=200)
    alphaLabel.place(x=20, y=150)

    alphaEntry = ctk.CTkEntry(lexWindow, font=font1, width=300)
    alphaEntry.place(x=210, y=151)

    # Cuadro de texto para la salida del análisis léxico con su scrollbar
    output_frame = ctk.CTkFrame(lexWindow, width=400, height=250)
    output_frame.place(x=20, y=220)

    output_text = ctk.CTkTextbox(output_frame, wrap="word", width=380, height=230, font=("Consolas", 10))
    output_text.pack(side="left", fill="both", expand=True)
    output_text.insert("end", "Resultados del análisis léxico:\n")

    scrollbar_output = ctk.CTkScrollbar(output_frame, command=output_text.yview)
    scrollbar_output.pack(side="right", fill="y")
    output_text.configure(yscrollcommand=scrollbar_output.set)

    # Cuadro de texto para los errores de análisis con su scrollbar
    error_frame = ctk.CTkFrame(lexWindow, width=300, height=250)
    error_frame.place(x=450, y=220)

    error_text = ctk.CTkTextbox(error_frame, wrap="word", width=280, height=230, font=("Consolas", 10), fg_color="#FFCCCC")
    error_text.pack(side="left", fill="both", expand=True)
    error_text.insert("end", "Errores:\n")

    scrollbar_error = ctk.CTkScrollbar(error_frame, command=error_text.yview)
    ...
    symbol_frame = ctk.CTkFrame(lexWindow, width=300, height=250)
    symbol_frame.place(x=800, y=220)

    symbol_text = ctk.CTkTextbox(symbol_frame, wrap="word", width=280, height=230, font=("Consolas", 10), fg_color="#E6FFCC")
    symbol_text.pack(side="left", fill="both", expand=True)
    symbol_text.insert("end", "Tabla de símbolos:\n")

    scrollbar_symbol = ctk.CTkScrollbar(symbol_frame, command=symbol_text.yview)
    scrollbar_symbol.pack(side="right", fill="y")
    symbol_text.configure(yscrollcommand=scrollbar_symbol.set)

# Ejecución de la ventana principal
root = ctk.CTk()
root.title("Analizador Léxico")
ctk.CTkButton(root, text="Abrir Prueba", command=Prueba).pack(pady=20)
root.mainloop()