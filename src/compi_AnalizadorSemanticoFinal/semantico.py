...
def analizadorSemanticoFinal():
    ...
    VentanaPrincipal.config(background="#27445D")
...
def encabezado(VentanaPrincipal):
    ...
    archivoLabel=Label(VentanaPrincipal,text="Seleccionar Archivo:",font=font1,width=20,background="#A89C29",foreground="white")
    archivoLabel.place(x=60,y=30)
    archivoButton=Button(VentanaPrincipal,text="1 Cargar Gramatica",width=20,command=lambda:abrirArchivo(VentanaPrincipal),bg="#497D74",font=font1)
    archivoButton.place(x=300,y=20)
    tokenButton=Button(VentanaPrincipal,text="2 Abrir Archivo",width=20,bg="#497D74",font=font1,command=lambda:abrirArchivo1(VentanaPrincipal))
    tokenButton.place(x=300,y=100)
    ImprimirResultad0s=Button(VentanaPrincipal,text="3 Imprimir Resultados",width=20,bg="#497D74",font=font1,command=lambda:imprimirResultados(VentanaPrincipal))
    ImprimirResultad0s.place(x=700,y=20)
    limpiarButton=Button(VentanaPrincipal,text="Limpiar",width=20,bg="#497D74",font=font1,command=lambda:limpiar(VentanaPrincipal))
    limpiarButton.place(x=900,y=20)
    nuevoArchivoButton=Button(VentanaPrincipal,text="4 Generar Código",width=20,bg="#497D74",font=font1,command=lambda:nuevoArchivo())
...
def abrirArchivo(Ventana):
    ...
    ruta_proyecto = r"./"
    direccionArchivo2= "entradaLR_trad.txt"    #  Antes solo aceptaba una gramática, esto se tuvo que cambiar
...
def abrirArchivo1(Ventana):
    ...
    Ventana.grab_set()
    username=getpass.getuser()
    ruta_proyecto = r"../../Pruebas_Analizador_Semantico/CodigosJava/"
...
def TablaLr(variable,simbolos,tira,arreGramatica,Ventana,arreAcciones):
    ...
    pila.append(0)
    font1=("Times New Roman",14)
    labelTextPila=Label(tabla,text="Pila",width=20,background="#A89C29",font=font1,borderwidth=2,relief="solid")
    labelTextPila.grid(row=contadorFila,column=0)
    labelTextTira=Label(tabla,text="Entrada",width=30,background="#A89C29",font=font1,borderwidth=2,relief="solid")
    labelTextTira.grid(row=contadorFila,column=1)
    labelTextSalida=Label(tabla,text="Salida",width=100,background="#A89C29",font=font1,borderwidth=2,relief="solid")    # Se le da más espacio a la salida por las tres columnas que debe almacenar
